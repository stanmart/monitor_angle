...
def create_setup_ui() -> tuple[Column, Widgets]:
    ...
    viewing_distance_slider = Slider(
        start=30, end=150, value=60, step=10, title="Viewing distance (cm)"
    )
    ...
    size_input = Slider(
        value=24, start=10, end=60, step=1, title="Monitor size (inches)"
    )
    size_row = row(size_input, aspect_box)

    curved_checkbox = CheckboxButtonGroup(labels=["Curved"], active=[])
    radius_slider = Slider(
        start=500, end=3000, value=1500, step=100, title="Radius (mm)", disabled=True
    )
...
def create_default_plot() -> tuple[
    figure, GlyphRenderer, LinearColorMapper, list[MonitorData]
]:
    """Create the plot for displaying the monitor positions."""
    default_monitor = MonitorData(
        diagonal_length=24, radius=inf, aspect_h=16, aspect_v=9
    )
    default_setup = SetupData(
        num_monitors=2, alignment="perpendicular", viewing_distance=60
    )
    default_monitor_data = [default_monitor] * default_setup.num_monitors

    setup = create_setup(default_setup, default_monitor_data)
    fig, line, colormap = setup.plot(width=600, show_colorbar=True)

    return (fig, line, colormap, default_monitor_data)
...
def create_setup(setup_data: SetupData, monitor_data: list[MonitorData]) -> Setup:
    ...
    monitors = [
        Monitor(
            diagonal_length=monitor.diagonal_length,
    ...
    setup = Setup(
        monitors,
        mode=setup_data.alignment,
        viewing_distance=setup_data.viewing_distance,
    )
    return setup
...
def create_column(title: str = "") -> Column:
    def onchange_setting(attr, old, new) -> None:
        ...
        changed_monitor = MonitorData(
            ...
            radius=widgets.radius.value if 0 in widgets.curved.active else inf,  # type: ignore # noqa
    ...
    def onchange_monitor_num(attr, old, new) -> None:
        ...
        if new > old:
            for _ in range(new - old):
                monitor_data.append(
                    MonitorData(diagonal_length=24, radius=inf, aspect_h=16, aspect_v=9)
                )
        else:
    ...
    for widget in [
        widgets.viewing_distance,
        widgets.monitor_size,
        widgets.aspect_h,
        widgets.aspect_v,
        widgets.radius,
    ]:
